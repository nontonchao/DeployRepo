...
const checkTimeSlot = async (date, eventCategoryId) => {
  ...
  slot_t.forEach((x) => {
    if (x.eventStartTime != d_tmp.value) {
...
const generateTimeSlot = (eventDuration) => {
  ...
  var active_date = new Date(props.eventz.eventStartTime);
  for (let i = 0; i < timeTable.value.length; i++) {
    if (timeTable.value[i].includes(active_date)) {
      activeIndex.value = i;
      time.value = timeTable.value[i].split('-')[0].trim();
    }
  }
