...
const addEventList = async () => {
    toSend.value = {
        ...
        "eventDuration": JSON.stringify(props.cliniclist.filter(x => x.eventCategoryName === clinicX.value)[0].eventDuration),
        "eventNotes": note.value,
        "eventCategory": {
            "id": JSON.stringify(props.cliniclist.filter(x => x.eventCategoryName === clinicX.value)[0].id)
        }
    }
...
<template>
    <div class="w-full max-w-lg md:w-3/5 px-6 mb-8 container mx-auto content-center leading-8 ">
        <div>
            <label class="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2 m-5"
                for="grid-clinic">คลินิก :
            </label>
            <div class="relative">
            ...
            <div class="w-full md:w-full px-3 mb-6 md:mb-0">
                <label class="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2 m-3"
                    for="username">ชื่อผู้นัดหมาย : </label>
                <input type="text" v-model="name" required
                    class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                    id="grid-name" placeholder="ชื่อ">
            </div>
            <div class="w-full md:w-full px-3 mb-6 md:mb-0">
                <label class="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2 m-3"
            ...
            <div class="w-full md:w-full px-3 mb-6 md:mb-0">
                <label class="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2 m-3">วันเวลา : </label>
                <input type="datetime-local" v-model="startTime" required :min="datetime"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
        </div>
        <div class="w-full m-5 md:w-full px-3 mb-8 md:mb-0">
            <button @click="toggleQue()" :disabled="isFullfill()"
                class="bg-main  p-5 hover:text-black hover:bg-acqua text-white font-bold py-2 px-4 rounded-full"
                type="button">นัดหมาย</button>
        </div>
    </div>
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 space-y-6">
                    <h3 class="text-base leading-relaxed text-yellow-300 dark:text-gray-400"><strong>{{
                            clinicX
                    }}</strong>
                    </h3>
                ...
                <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                    <button data-modal-toggle="defaultModal" type="button" @click="addEventList(); resetModal()"
