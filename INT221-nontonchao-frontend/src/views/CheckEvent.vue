...
const editDateTime = async (updateEvent) => {
    ...
    const res = await fetch(`http://localhost/api/events/edit/`, {
        method: 'PUT',
        headers: {
...
const FindEventByEmail = async () => {
    ...
    const res = await fetch(`http://localhost/api/events/find/${email.value}`, {
        method: 'GET'
    });
    if (res.status == 200) {
        ret = await res.json();
...
const deleteEventFromId = async (id) => {
    ...
    const res = await fetch(`http://localhost/api/events/delete/${id}`, {
        method: 'DELETE'
    });
    if (res.status == 200) {
        eventList.value = await getAllEventList();
