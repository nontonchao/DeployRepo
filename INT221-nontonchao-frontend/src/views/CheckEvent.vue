  ...
  filter_list.value = eventList.value.filter(
    (x) =>
      ...
      (status.value == "ทั้งหมด" && selectDate.value == ""
        ? x
        : status.value == "ทั้งหมด" && selectDate.value != ""
        ? x.eventStartTime.split("T")[0] == selectDate.value
...
<template>
  <div
    class="w-full md:w-3/5 px-6 mb-8 md:mb-0 container mx-auto content-center leading-8"
  >
  ...
  <div
    v-show="eventStore.editCode == 400"
    id="defaultModal"
    tabindex="-1"
    aria-hidden="true"
  ...
  <AllEventList
    :status="fStatus"
    :eventList="filter_list"
    @delete="deleteEventFromId"
    @edit="editEvent"
  />
</template>
