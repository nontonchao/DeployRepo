...
const getAllEventList = async () => {
    ...
    const res = await fetch("http://backend:8080/api/events", {
        method: 'GET'
    });
    if (res.status == 200) {
        ret = await res.json();
...
const editDateTime = async (updateEvent) => {
    ...
    const res = await fetch(`http://backend:8080/api/events/edit/`, {
        method: 'PUT',
        headers: {
...
const FindEventByEmail = async () => {
    ...
    const res = await fetch(`http://backend:8080/api/events/find/${email.value}`, {
        method: 'GET'
    });
    if (res.status == 200) {
        ret = await res.json();
...
const deleteEventFromId = async (id) => {
    ...
    const res = await fetch(`http://backend:8080/api/events/delete/${id}`, {
        method: 'DELETE'
    });
    if (res.status == 200) {
        eventList.value = await getAllEventList();
